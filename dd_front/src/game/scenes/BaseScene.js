...
import ExhibitionManager from '../objects/ExhibitionManager';
...
export default class BaseScene extends Scene {
     ...
     * @param {boolean} [config.enableExhibition=true]
     */
    constructor({ key, mapData, debug = false, interactionUIConfig = {}, enableChatbot = true, enableExhibition = true }) {
        super({ key });
        this.mapData = mapData;
        this.debug = debug;
        this.enableChatbot = enableChatbot;
        this.enableExhibition = enableExhibition;
        this.exhibitionManager = null;
    ...
    create(data) {
        ...
        if (this.enableExhibition) {
            this._setupExhibitionSystem();
        }
    ...
    /**
     * Configure le système d'exposition pour la scène
     */
    _setupExhibitionSystem() {
        this.exhibitionManager = new ExhibitionManager(this);
        
        console.log('Système d\'exposition activé pour la scène:', this.scene.key);
        
        EventBus.emit('exhibition-system-ready', {
            sceneKey: this.scene.key,
            exhibitionManager: this.exhibitionManager
        });
    }
    ...
    _disablePhaserControls() {
        ...
        this.input.keyboard.on('keydown-ESC', () => {
            ...
            if (this.exhibitionManager && this.exhibitionManager.isAnyDialogOpen()) {
                return;
            }
        });
    }
    ...
    update() {
        if (!this.isChatbotOpen && !this.isMultiplayerChatOpen) {
            ...
            if (this.exhibitionManager) this.exhibitionManager.update();
        }
    }
    ...
    */
    ...
    _cleanup() {
        ...
        if (this.exhibitionManager) {
            this.exhibitionManager.destroy();
            this.exhibitionManager = null;
        }
